...
| `workflows.<name>.jobs.<name>.matrix` | ❌ | The `matrix` key allows running a specific job as part of a workload with different values, not supported yet by the Buildkite Migration tool. Consider translating specific matrix jobs to a [build matrix](https://buildkite.com/docs/pipelines/build-matrix) setup within a [command step](https://buildkite.com/docs/pipelines/command-step) |
