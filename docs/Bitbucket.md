...
| `pipelines.custom.<name>.import-pipeline.import` | ❌ | The specification of importing a pipeline from within a specific repository defined in top-level `definitions`. Consider using [trigger steps](https://buildkite.com/docs/pipelines/trigger-step) to invoke builds on a specific Buildkite pipeline. |
...
| `pipelines.<start-condition>.step.condition` | ⚠️ | The configuration to prevent a Bitbucket pipeline step from running unless the specific conditional is met. Translated to an inline conditional (`if`) within the corresponding Buildkite pipelines' command step's `commands` - based on a `git diff` of the base branch. |
